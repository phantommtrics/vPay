...
import { EmailNotificationStatus, EmailNotificationTemplate, type Prisma } from '@prisma/client';
import { z } from 'zod';
...
const emailNotificationQuerySchema = paginationSchema
  .extend({
    template: z.nativeEnum(EmailNotificationTemplate).optional(),
    status: z.nativeEnum(EmailNotificationStatus).optional(),
...
export async function handleAdminReportEmailNotifications(
  req: AdminAuthedRequest,
  res: Response,
): Promise<void> {
  const parsed = emailNotificationQuerySchema.safeParse(req.query);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.errors[0]?.message ?? 'Invalid query' });
    return;
  }
  ...
  const where: Prisma.EmailNotificationWhereInput = {
    ...
    ...(parsed.data.email
      ? { recipientEmail: { equals: parsed.data.email, mode: 'insensitive' } }
      : {}),
  };
  ...
  const rows = await prisma.emailNotification.findMany({
    ...
    include: {
      recipientUser: {
        select: { id: true, email: true, firstName: true, lastName: true, adminUser: true },
      },
    },
  });
  ...
  const notifications = slice.map((row) => ({
    id: row.id,
